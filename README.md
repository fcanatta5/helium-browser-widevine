...
## DRM / Netflix / Widevine on Linux

This Linux bundle has Widevine/EME support enabled at build level and includes a runtime helper for registering an existing `WidevineCdm` installation. Google's proprietary Widevine CDM is **not** bundled. See [`DRM-WIDEVINE.md`](DRM-WIDEVINE.md) for Arch/Linux setup, runtime variables, and packaging notes.
