...
if [ -r "${HERE}/opt/helium/helium-widevine.sh" ]; then
    # Register an already-installed WidevineCdm payload for DRM/EME sites such
    # as Netflix. This does not download or bundle proprietary files.
    . "${HERE}/opt/helium/helium-widevine.sh"
    helium_widevine_prepare "${HERE}/opt/helium" "$@"
fi
