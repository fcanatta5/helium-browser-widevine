...
cp "$_root_dir/package/helium-widevine.sh" "$_tarball_dir/helium-widevine.sh"

# Optional: when a local build intentionally bundles WidevineCdm into
# out/Default, ship it in the tarball/AppImage. Official Helium source bundles
# do not contain Google's proprietary CDM.
if [ -d "$_build_dir/src/out/Default/WidevineCdm" ]; then
    cp -r "$_build_dir/src/out/Default/WidevineCdm" "$_tarball_dir/WidevineCdm" &
fi
