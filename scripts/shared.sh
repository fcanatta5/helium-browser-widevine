...
write_gn_args() {
    ...
    # Optional local Widevine bundling. This is intentionally opt-in because
    # Google's Widevine CDM is proprietary and must not be redistributed unless
    # the packager has the right to do so. Runtime registration of an existing
    # system WidevineCdm install works without this.
    if [ "${HELIUM_BUNDLE_WIDEVINE:-0}" = 1 ]; then
        echo 'bundle_widevine_cdm = true' | tee -a "${_out_dir}/args.gn"
        if [ -n "${HELIUM_WIDEVINE_ROOT:-}" ]; then
            echo "widevine_root = "${HELIUM_WIDEVINE_ROOT}"" | tee -a "${_out_dir}/args.gn"
        fi
    fi
